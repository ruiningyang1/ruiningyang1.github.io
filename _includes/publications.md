...
<div class="pub-row">
  ...
  <div class="col-sm-9" style="position: relative;padding-right: 15px;padding-left: 20px;">
      <div class="title">{% if link.landing %}<a href="{{ link.landing }}">{{ link.title }}</a>{% elsif link.pdf %}<a href="{{ link.pdf }}">{{ link.title }}</a>{% else %}{{ link.title }}{% endif %}</div>
      <div class="author">{{ link.authors }}</div>
      <div class="periodical"><em>{{ link.conference }}</em>
      </div>
    <div class="links">
      {% if link.landing %}
      <a href="{{ link.landing }}" class="btn btn-sm z-depth-0" role="button" style="font-size:12px;">Details</a>
      {% endif %}
      {% if link.pdf %} 
